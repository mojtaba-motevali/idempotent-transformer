...
Then('the original task result should not be stored in plain text', async function () {
  const persisted = await storage.find(taskUniqueId);
  const compressor = new ZstdCompressor();
  const decompressed = await compressor.decompress(persisted!);
  const deserialized: IdempotencyResult<string> =
    await MessagePack.getInstance().deserialize(decompressed);
  // The persisted data should not contain the plain text
  expect({
    executionInputHash: await transformer.createHash(taskInput),
    executionResult: taskResult,
  }).to.deep.equal(deserialized);
});
